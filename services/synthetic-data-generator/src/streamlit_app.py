...
with tab2:
    ...
    if latest_df is None:
        ...
        with col_a:
            st.markdown("### Feature Drift Detection (KS Test)")
            st.caption("Compares distributions. p-value < 0.05 indicates significant drift.")
            ...
            for feat in test_features[:10]: # Limit to 10 for UI cleaniness
                if feat in latest_df.columns:
                    ...
                    if len(base_data) > 0 and len(latest_data) > 0:
                        stat, pval = ks_2samp(base_data, latest_data)
                        status = "🚨 Drifted" if pval < 0.05 else "✅ Stable"
                        results.append({"Feature": feat, "P-Value": pval, "Status": status})
        ...
        with col_b:
            ...
            base_nulls = baseline_df[test_features[:10]].isnull().mean() * 100
